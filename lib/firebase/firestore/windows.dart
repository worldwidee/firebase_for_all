...
Future<QuerySnapshotForAll<T>> getCollectionsWindows<T extends Object?>(CollectionReference ref,
    {required QueryProperties queryProperties, required ColRef<T> colRef}) async {
  dynamic init = ref;
    ...
    if (value.isNotEmpty) {
      snapshots = QuerySnapshotForAll<T>(
          List<DocumentSnapshotForAll<T>>.from(value
              .map((element) => DocumentSnapshotForAll<T>(element.map, element.id,
...
Future<DocumentSnapshotForAll<T>> getDocumentWindows<T extends Object?>(DocumentReference ref,
